...
def _tension_label(clubs: int, players: int) -> tuple[str, str, str]:
    ...
    demand-per-supply ratio. Bands match the engine's market_tension_multiplier
    in scripts/22_match_engine.py (>= 1.3 tight, 0.7-1.3 balanced, < 0.7 loose).
    Same thresholds as the position tension table; ratios reconcile across the app.
    """
...
def _overview_data(engine_key: str) -> dict[str, dict]:
    """For each bucket, count available players in the active engine's cohort.

    `engine_key` keys the cache so Brokerage / Market View overview cards
    don't share state. Filter shape mirrors `db.active_cohort_filter()`:
      • brokerage → sellable_now ∩ brokerage_eligible (small per-bucket counts)
      • market_view → sellability ≥ 35 ∩ has CA ∩ NOT IFA — same numbers as
        the engine's tension table (GK 282, CB 583, LB 252 …)
    """
    if engine_key == db.ENGINE_BROKERAGE:
        cohort_sql = ("sellability_status = 'sellable_now' "
                      "AND brokerage_eligible = 1 "
                      "AND COALESCE(is_imminent_free_agent, 0) = 0")
    else:
        # Tightened to match the engine's tension supply count exactly —
        # CA required on every row, both branches (sellable_now without CA
        # was leaking 1-3 rows per bucket vs the engine numbers).
        cohort_sql = (
            "COALESCE(is_imminent_free_agent, 0) = 0 "
            "AND EXISTS (SELECT 1 FROM player_ratings pr2 "
            "            WHERE pr2.tm_player_id = player_universe.player_id "
            "              AND pr2.current_ability IS NOT NULL) "
            "AND (sellability_score >= 35 OR sellability_status = 'sellable_now')"
        )
    con = db.get_connection()
    out: dict[str, dict] = {}
    excluded = db.get_excluded_ids()
    for b in ALL_BUCKETS:
        n_players_universe = con.execute(f"""
        ...
        # Distinct clubs looking at this position — used for the secondary
        # "X clubs looking" label on the card. Demand-mapped leagues only.
        ...
        # Raw request count (UNION ALL — duplicates kept, all 19 leagues).
        # 2026-06-09: this is the new headline ratio denominator-source so
        # the card's "X× demand" reconciles exactly with the engine tension
        # table + Market Overview's "All" column. Single source of truth.
        n_requests_raw = con.execute("""
            SELECT COUNT(*) FROM (
                SELECT 1 FROM map_club_requests WHERE position_bucket = ?
                UNION ALL
                SELECT 1 FROM inferred_club_requests WHERE position_bucket = ?
            )
        """, (b, b)).fetchone()[0]
...
if bucket is None:
    ...
    data = _overview_data(db.active_engine())
    ...
    for b in ALL_BUCKETS:
        ...
        # Headline ratio uses raw request count (engine tension definition)
        # so the card reconciles exactly with Market Overview "All" column.
        tension_lbl, tension_colour, _ = _tension_label(d["n_requests"], d["n_players"])
        ratio_str = (f"{d['n_requests'] / d['n_players']:.2f} demand÷supply"
...
# ─── Pull data ───────────────────────────────────────────────────────────────
data = db.get_position(bucket, sort_col=db.active_match_score_col(), engine_key=db.active_engine())
...
if not len(matches_for_table):
    ...
    mdf["methodology"] = mdf.apply(
        lambda r: ui.build_methodology(r, score_col=db.active_match_score_col()), axis=1
    )
    ...
    mdf = mdf.sort_values(db.active_match_score_col(), ascending=False, kind="stable").reset_index(drop=True)
    ...
    show_df = table_df[[
        ...
        "match_score", "sellability_score", "methodology",
    ]].rename(columns={
        "Player_html":          "Player",
        "current_club_display": "Current Club",
    ...
    show_df["Match Score"] = show_df["Match Score"].apply(ui.fmt_score_capped)
    show_df["Sellability"] = show_df["Sellability"].apply(ui.fmt_score_capped)
