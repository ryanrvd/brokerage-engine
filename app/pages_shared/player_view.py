...
if not raw_pid:
    ...
    # Engine-aware cohort (2026-06-08 fix):
    #   Brokerage Engine → strict sellable_now ∩ brokerage_eligible (~90 today)
    #   Market View      → wider mandate-relevant cohort: sellability ≥ 35 +
    #                      has CA + NOT IFA (~3,000 today)
    # The active filter comes from db.active_cohort_filter() so the page
    # auto-resizes when the user switches engines via the sidebar link.
    matches_all = db.get_all_matches(sort_col=db.active_match_score_col())
    ...
    cohort_rows = con.execute(f"""
        SELECT pu.player_id, pu.name, pu.age, pu.position_bucket,
               ...
               pu.parent_club, pu.parent_club_id,
               pu.league_id, pu.sellability_score, pu.on_loan
        FROM player_universe pu
        WHERE {db.active_cohort_filter()}
    """).fetchall()

    rows = []
    for (pid, official, age, bucket, current_club, current_club_id,
         parent_club, parent_club_id, league_id, sell, on_loan) in cohort_rows:
        rows.append({
            "player_id":          int(pid),
            "display_name":       labels.player_display_name(int(pid), official),
            "age":                age,
            ...
            "parent_league_id":   league_id,
            "sellability":        round(sell or 0.0, 1),
            "n_matches":          match_counts.get(int(pid), 0),
    ...
    with st.sidebar:
        ...
        # Engine in the widget key forces a fresh slider instance (and fresh
        # min/max defaults) when the user switches engines — otherwise the
        # widget reuses the previous engine's bounds via session-state cache.
        sel_sell = st.slider(
            "Sellability score", sell_min, sell_max, (sell_min, sell_max), 1,
            key=f"pv_sellability_{db.active_engine() or 'none'}",
        )
    ...
    # ─── Top stats ───────────────────────────────────────────────────────
    # "Total in cohort" reflects the active engine's cohort, not the wider
    # 11,809-row player_universe. Brokerage cohort ≈ 90; Market View cohort
    # ≈ 3,000 today.
    c1, c2, c3 = st.columns(3)
    c1.metric("Players shown", len(filtered))
    c2.metric(f"Total in cohort ({db.active_engine_label()})", len(df_all))
    ...
    def _bg_sell(v):
        # heatmap_gradient is cap-aware — accepts "100+", floats, NaN, None.
        return ui.heatmap_gradient(v, 0, 100)
...
data = db.get_player(pid, sort_col=db.active_match_score_col())
...
# Sort/top by the active engine's score column (match_score in Brokerage,
# market_match_score in Market View). The Market View cohort contains
# players whose match_score column is NULL — guard against an empty .idxmax().
_score_col = db.active_match_score_col()
_scores = matches[_score_col].dropna() if n_matches and _score_col in matches.columns else None
top_match    = float(_scores.max()) if _scores is not None and not _scores.empty else None
top_buyer_id = None
top_buyer_name = ""
if _scores is not None and not _scores.empty:
    top_row = matches.loc[_scores.idxmax()]
...
if n_matches == 0:
    ...
    # Methodology cell — full-word multiplicative chain from persisted match
    # components (set by scripts/22_match_engine.py).
    table_df["methodology"] = table_df.apply(
        lambda r: ui.build_methodology(r, score_col=db.active_match_score_col()),
        axis=1,
    )
    ...
    display = table_df.assign(rank=range(1, len(table_df) + 1))[[
        ...
        "match_score", "market_match_score", "methodology",
    ]].rename(columns={
        "rank":                 "#",
        "Buyer_html":           "Buyer",
    ...
    # Score columns: cap displayed values at 100 ("100+" beyond). Raw values
    # still drive sort + the Methodology cell's arithmetic chain.
    ...
    def _bg_match(v):
        # Pass `v` directly — heatmap_gradient is cap-aware (handles "100+",
        # numbers, NaN, None). Pre-converting via float() raises ValueError
        # on "100+" and the local except clause swallowed the cap-aware path.
        return ui.heatmap_gradient(v, 0, 100)

    styled = display.style.map(_bg_match, subset=["Brokerage Score", "Market Score"])
