...
Layout (top → bottom):
  ...
  4. Seller-side section (hidden if no sellable players):
       Sellable Assets table (full squad, sortable, toggle for ≥ threshold only)
       + Imminent Free Agents panel (rendered only when any exist)
...
if not raw_cid:
    ...
    for r in club_rows:
        ...
        # Defensive wrap (NoneType.__getitem__ guard): COUNT(*) should always return
        # a row but Streamlit's cache/state edge cases have surfaced fetchone()=None.
        _r = con.execute(
            "SELECT COUNT(*) FROM matches m JOIN player_universe pu ON pu.player_id=m.player_id "
            "WHERE pu.parent_club_id = ?", (d["club_id"],)
        ).fetchone()
        d["matches_as_seller"] = _r[0] if _r else 0
    ...
    display["Selling Pressure"] = display["Selling Pressure"].apply(ui.fmt_score_capped)
...
data = db.get_club(cid, sort_col=db.active_match_score_col())
...
# Squad size — from senior_roster (defensive fetchone() wrap)
con = db.get_connection()
_r = con.execute(
    "SELECT COUNT(*) FROM senior_roster WHERE club_id = ? AND minutes_last_18m > 0",
    (cid,),
).fetchone()
squad_size = _r[0] if _r else 0
if squad_size == 0:
    # Fall back to total roster headcount (for external/second-tier clubs without minutes)
    _r2 = con.execute("SELECT COUNT(*) FROM senior_roster WHERE club_id = ?", (cid,)).fetchone()
    squad_size = _r2[0] if _r2 else 0
...
# ─── Seller-side section (full squad sorted by sellability, IFAs separated) ─

# Split the parented squad into the active sellable cohort + IFA cohort.
# Imminent free agents are NOT brokerage opportunities (Bosman pre-contract
# eligible — see scripts/09_compute_sellability.py). They surface in a
# dedicated panel beneath the main Sellable Assets table.
if "is_imminent_free_agent" in sellable.columns:
    ifa_mask    = sellable["is_imminent_free_agent"].fillna(0).astype(int) == 1
    ifa_df      = sellable[ifa_mask].copy()
    non_ifa_df  = sellable[~ifa_mask].copy()
else:
    ifa_df, non_ifa_df = sellable.iloc[0:0].copy(), sellable.copy()

SELLABILITY_THRESHOLD = config.MANDATE_COHORT_SELLABILITY_FLOOR  # 35.0 (Phase A.8.7)
squad_total       = len(non_ifa_df)
n_above_threshold = int((non_ifa_df["sellability_score"] >= SELLABILITY_THRESHOLD).sum()) if squad_total else 0

if squad_total > 0:
    st.markdown(f"## Sellable Assets ({n_above_threshold} of {squad_total})")
    st.caption(
        "Squad players sorted by sellability score. Toggle to see the full "
        "squad including below-threshold players."
    )
    show_above_only = st.checkbox(
        f"Show only above sellability threshold ({int(SELLABILITY_THRESHOLD)})",
        value=True,
        key=f"club_view_above_only_{cid}",
    )

    # (Removed 2026-06-04: the "Top 3 likely to move" preview row was redundant —
    # the same names sit at the top of the Sellable Assets table directly below.)

    # Build display from the full (non-IFA) squad
    sellable_df = non_ifa_df.copy()
    if show_above_only:
        sellable_df = sellable_df[sellable_df["sellability_score"] >= SELLABILITY_THRESHOLD].reset_index(drop=True)

    if sellable_df.empty:
        st.info(
            "No players in this squad above the sellability threshold. "
            "Untick the toggle above to see the full squad."
        )
    else:
        ...
        # Buyer-match counts are engine-aware: only count rows above the active
        # lens's floor. Brokerage uses match_score; Market View uses
        # market_match_score. Out-of-cohort players (sellability < 50 AND not
        # 'sellable_now') get a muted-grey "out of cohort" cell — they don't
        # surface in the matches table at all, so the count would be meaningless.
        _active_col = db.active_match_score_col()
        _floor = 10.0 if _active_col == "match_score" else 5.0
        match_counts = dict(con2.execute(
            f"SELECT player_id, COUNT(*) FROM matches "
            f"WHERE {_active_col} >= {_floor} "
            f"  AND player_id IN ({','.join(str(int(x)) for x in sellable_df['player_id'])}) "
            f"GROUP BY player_id"
        ).fetchall()) if len(sellable_df) else {}

        def _buyer_count_cell(r) -> str:
            sell = r.get("sellability_score") or 0
            status = r.get("sellability_status") or ""
            # Out-of-cohort trigger uses the new 35 sellability floor (Phase A.8.7).
            if sell < config.MANDATE_COHORT_SELLABILITY_FLOOR and status != "sellable_now":
                return '<span style="color:#9CA3AF; font-style:italic;">out of cohort</span>'
            n = match_counts.get(int(r["player_id"]), 0)
            return str(n)

        sellable_df["buyer_count"] = sellable_df.apply(_buyer_count_cell, axis=1)
...
n_ifa = len(ifa_df)
if n_ifa > 0:
    st.markdown(f"### 🛫 Imminent Free Agents ({n_ifa})")
    st.caption(
        "Players within 6 months of contract end — Bosman pre-contract eligible. "
        "Not surfaced as seller-side brokerage opportunities. Possible player-side "
        "mandate opportunity."
    )
    ifa_disp = ifa_df.copy()
    pname_lookup_ifa = {p["player_id"]: p["display_name"] for p in db.get_player_search_options()}
    ifa_disp["player_display"] = ifa_disp.apply(
        lambda r: pname_lookup_ifa.get(int(r["player_id"]), r["name"]), axis=1
    )
    ifa_disp["Player_html"] = ifa_disp.apply(
        lambda r: ui.player_link(int(r["player_id"]), str(r["player_display"])),
        axis=1,
    )
    ...
    def _days_left(ce):
        if not ce or pd.isna(ce): return None
        try:
            return (datetime.fromisoformat(str(ce)).date() - snapshot).days
        except (TypeError, ValueError):
            return None
    ifa_disp["days_left"] = ifa_disp["contract_end_date"].apply(_days_left)
    ifa_disp["mv_str"]    = ifa_disp["current_tm_value_eur"].apply(_money)
    ifa_disp["badge"]     = (
        '<span style="background:#FEF3C7; color:#92400E; padding:1px 8px; '
        'border-radius:5px; font-size:0.75rem; font-weight:700;">🛫 Bosman</span>'
    )

    # CA/PA from player_ratings
    con_r = db.get_connection()
    ca_rows = con_r.execute(
        f"SELECT tm_player_id, current_ability, potential_ability "
        f"FROM player_ratings WHERE tm_player_id IN ({','.join(str(int(x)) for x in ifa_disp['player_id'])})"
    ).fetchall() if len(ifa_disp) else []
    ca_map = {int(r[0]): (r[1], r[2]) for r in ca_rows}
    ifa_disp["ca_str"] = ifa_disp["player_id"].apply(
        lambda pid: f"{ca_map.get(int(pid), (None,None))[0]:.0f}" if ca_map.get(int(pid), (None,None))[0] is not None else "—"
    )
    ifa_disp["pa_str"] = ifa_disp["player_id"].apply(
        lambda pid: f"{ca_map.get(int(pid), (None,None))[1]:.0f}" if ca_map.get(int(pid), (None,None))[1] is not None else "—"
    )

    ifa_disp = ifa_disp.sort_values("days_left", ascending=True).reset_index(drop=True)

    show_ifa = ifa_disp[[
        "Player_html", "position_bucket", "age", "contract_end_date",
        "days_left", "mv_str", "ca_str", "pa_str", "badge",
    ]].rename(columns={
        "Player_html":        "Player",
        "position_bucket":    "Position",
        "age":                "Age",
    ...
    show_ifa["Position"] = show_ifa["Position"].apply(labels.display_bucket)
    show_ifa["Days Remaining ▲"] = show_ifa["Days Remaining ▲"].apply(
        lambda v: f"{int(v)}" if pd.notna(v) else "—"
    )
    ui.render_html_table(show_ifa.style, max_height_px=min(440, 60 + 36 * n_ifa))
    st.markdown("")
...
if buyer_side_has_content:
    # Visual divider — only when the seller-side block above rendered anything
    # (squad_total = full parented squad excluding IFAs; defined in the
    # Sellable Assets block above).
    if squad_total > 0 or n_ifa > 0:
    ...
    # Sub B — Matches as Buyer (filter Kill List + engine-active score floor)
    if n_buyer_mx > 0:
        excluded_ids = db.get_excluded_ids()
        mx_df = buyer_mx[~buyer_mx["player_id"].isin(excluded_ids)].copy()
        # Filter to rows where the active engine's score is at or above the
        # display threshold (20). Below-floor rows are noise — they wouldn't
        # surface elsewhere in the engine, so we don't surface them here either.
        _active_col_mxb = db.active_match_score_col()
        if _active_col_mxb in mx_df.columns:
            mx_df = mx_df[mx_df[_active_col_mxb].fillna(0) >= 25].copy()
        if len(mx_df):
            st.markdown(
            ...
            mx_df["methodology"] = mx_df.apply(
                lambda r: ui.build_methodology(r, score_col=_score_col), axis=1
            )
            mx_sorted = (
                mx_df.dropna(subset=[_score_col])
            ...
            # Engine-aware Match Score column: render whichever score column
            # the active lens uses (match_score for Brokerage, market_match_score
            # for Market View). Hardcoded match_score rendered NULL for every
            # Market-View-only buyer-match row.
            mx_show = (
                mx_sorted
                ...
                [["rank", "Player_html", "age", "position_bucket", "bucket_tier",
                  ...
                  _score_col, "sellability_score", "methodology"]]
                .rename(columns={
                    "rank":                  "#",
                    "Player_html":           "Player",
                    "age":                   "Age",
                    ...
                    _score_col:              "Match Score ▼",
                    "sellability_score":     "Sellability",
                    "methodology":           "Methodology",
                })
            )
            mx_show["Match Score ▼"] = mx_show["Match Score ▼"].apply(ui.fmt_score_capped)
            mx_show["Sellability"]   = mx_show["Sellability"].apply(ui.fmt_score_capped)
