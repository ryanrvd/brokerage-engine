...
Cloned from home.py (Targets) with four meaningful differences:
  ...
  1. Data source: db.get_all_matches() — no Kill List filter applied.
...
_ms_lo = int(df["match_score"].min())
_ms_hi = int(df["match_score"].max()) + 1
...
filtered = filtered[
    ...
    & filtered["match_score"].between(f_ms_range[0], f_ms_range[1])
...
if len(filtered):
    filtered = filtered.sort_values("match_score", ascending=False, kind="stable")
...
if len(filtered):
    filtered["is_top5_overall"] = (
        filtered["match_score"].rank(method="first", ascending=False) <= 5
    )
else:
...
if len(df):
    top_row = df.iloc[df["match_score"].idxmax()]
    top_score = float(top_row["match_score"])
