...
def _net_role(supply: int, demand: int, mapped: bool) -> tuple[str, str, str]:
    """Return (label, css-colour, opportunity-framing). League analogue of
    Position View's _tension_label — same structural pattern: name who has
    the upper hand, give a brokerage-relevant opportunity sentence."""
    if not mapped:
        return (
            "Supply only",
            "#6b7280",
            "Demand for this league not currently mapped. Sell-side only.",
        )
    ...
    if demand <= 0:
        return (
            "Supply only",
            "#6b7280",
            "Players available but no buyer demand recorded in this window.",
        )
    if supply <= 0:
        return (
            "Net buyer",
            "#1d4ed8",
            "Inflow league — clubs actively recruiting. Strong buy-side broker opportunity.",
        )
    ratio = supply / demand
    if ratio >= 1.3:
        return (
            "Net supplier",
            "#b91c1c",
            "Outflow league — premium-priced exits. Strong sell-side positioning.",
        )
    if ratio <= 0.7:
        return (
            "Net buyer",
    ...
    return (
        "Balanced",
        "#15803d",
        "Two-way flow. Standard deal flow in both directions.",
    )


role_label, role_colour, role_opportunity = _net_role(
    n_sellable_actionable, n_buyer_requests, is_demand_mapped
)

# Top Match — highest match score where a player FROM THIS LEAGUE is being
# sold (sell-side perspective). Kill-List-excluded. Restricted to mapped-
# buyer matches (already applied above).
top_match_row = None
seller_side_matches = matches_actionable[matches_actionable["parent_league"] == lg]
if len(seller_side_matches):
    top_match_row = seller_side_matches.iloc[seller_side_matches["match_score"].idxmax()]
...
tiles = [
    ...
    _tile("Buyer requests", buyer_requests_value,
          ...
          title="Buyer requests originating from clubs in this league. "
                ...
                "leagues read as '—'."),
    _tile("Net role",
          f'<span style="color:{role_colour};">{role_label}</span>',
          subline=(
              f'<div style="color:#4b5563; font-weight:500; font-size:0.78rem; '
              f'margin-top:4px; line-height:1.35; white-space:normal;">'
              f'{role_opportunity}</div>'
          ),
          title="Net role of this league — supply ÷ demand. ≥1.3 = net supplier, "
                "≤0.7 = net buyer, in between = balanced. Non-demand-mapped "
                "leagues are supply-only."),
...
def _build_commentary() -> str:
    ...
    parts = []
    parts.append(
        f"{league_display} — {role_label.lower()}. {n_sellable_actionable} "
        f"sellable players against {n_buyer_requests} buyer requests from "
        f"{league_display} clubs."
    )
    ...
    # Sentence 5 — closing line per net role
    closing = {
        "Net supplier": "Standard broker opportunity — fees from this league stay premium.",
        "Net buyer":    "Active recruitment — clubs from this league are scanning for upgrades.",
        "Balanced":     "Two-way market — pathways exist in both directions.",
    }.get(role_label, "")
    if closing:
...
if not len(sellers_for_table):
    ...
    def _top_lf(pid: int) -> str | None:
        r = con.execute(
            ...
            "ORDER BY match_score DESC LIMIT 1", (int(pid),)).fetchone()
    ...
    show_df["Best match"]  = show_df["Best match"].apply(lambda v: f"{v:.1f}" if pd.notna(v) and v else "—")
...
if not pressured_rows:
    ...
    def _bg_pressure(v):
        try: return ui.green_gradient(float(v), _p_min, _p_max)
        except (TypeError, ValueError): return ""
